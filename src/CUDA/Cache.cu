...
namespace CUDA
{
	namespace Cache
	{
		...
		void TestCache()
		{
			VisualDebugging::AddLine("axes", { 0, 0, 0 }, { 25.0f, 0.0f, 0.0f }, Color4::Red);
			VisualDebugging::AddLine("axes", { 0, 0, 0 }, { 0.0f, 25.0f, 0.0f }, Color4::Green);
			VisualDebugging::AddLine("axes", { 0, 0, 0 }, { 0.0f, 0.0f, 25.0f }, Color4::Blue);
			...
			for (size_t i = 0; i < ply.GetPoints().size() / 3; i++)
			{
				...
				VD::AddSphere("Points", { x, y, z }, 0.05f, Color4::FromNormalized(r, g, b, 1.0f));
			}

			t = Time::End(t, "Loading");

			VD::AddCube("test", { 0.0f, 0.0f, 0.0f }, 25.0f);

			for (size_t zIndex = 0; zIndex < 500; zIndex++)
			{
				if (1 < zIndex && 498 > zIndex) continue;
				for (size_t yIndex = 0; yIndex < 500; yIndex++)
				{
					if (1 < yIndex && 498 > yIndex) continue;
					for (size_t xIndex = 0; xIndex < 500; xIndex++)
					{
						if (1 < xIndex && 498 > xIndex) continue;

						float x = (float)(xIndex)*voxelSize - 25.0f;
						float y = (float)(yIndex)*voxelSize - 25.0f;
						float z = (float)(zIndex)*voxelSize - 25.0f;

						VD::AddCube("voxels", { x,y,z }, 0.05f);

						printf("[%d, %d, %d] %f, %f, %f\n", xIndex, yIndex, zIndex, x, y, z);
					}
				}
			}
		}
