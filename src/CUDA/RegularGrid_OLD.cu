...
#pragma endregion
		...
		void SaveRegularGridToVTK(const RegularGrid<Voxel>& rg, const std::string& filename) {
			...
			Voxel* elements = new Voxel[rg.h_internal.numberOfVoxels];
			cudaMemcpy(elements, rg.h_internal.elements, sizeof(Voxel) * rg.h_internal.numberOfVoxels, cudaMemcpyDeviceToHost);
			checkCudaErrors(cudaDeviceSynchronize());
