...
void LoadPLYPatch(int patchID, Eigen::AlignedBox3f& aabb, vector<Eigen::Vector3f>& loadedPoints, int& loadedCount)
{
	aabb = Eigen::AlignedBox3f(Eigen::Vector3f(FLT_MAX, FLT_MAX, FLT_MAX), Eigen::Vector3f(-FLT_MAX, -FLT_MAX, -FLT_MAX));
	loadedPoints.clear();
	loadedPoints.resize(256 * 480);
	loadedCount = 0;

	stringstream ss;
	ss << "C:\\Resources\\2D\\Captured\\PointCloud\\point_" << patchID << ".ply";
...
int main()
{
	...
	app.AddAppStartCallback([&](App* pApp) {
		...
		unordered_map<VoxelKey, Voxel> volume;
